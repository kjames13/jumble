...
<?php
     ...
     * A valid string has a length greater than zero and only contains English letters.
     * The longest valid English word is 45 characters long, but I did not include this as a parameter to
     * check if the string is valid. If the user enters a string longer than 45 characters, than they just
     * will not generate any valid English words in the resultant table.
     */
    ...
    function generate_perms($prefix, $chars, &$perms) {
        //base case
        if (count($chars) == 1) {
            $new = $prefix . array_pop($chars);
            //only add to the list of perms if it does not already exist (to avoid repeats)
            if (!in_array($new, $perms)) {
                $perms[] = $new;
            } 
        }
        else {
